...
try {
    ...
    $runRoot = Join-Path $workspaceRoot 'training-runs\hazard5v3s_yamnet1024'
    $mlflowRoot = Join-Path $workspaceRoot 'training-runs\mlruns-hazard5v3s-yamnet1024-short'
    ...
    foreach ($requiredPath in @(
        ...
        $pretrainedModel,
        (Join-Path $workspaceRoot 'datasets\hazard5v3s\audio'),
        (Join-Path $workspaceRoot 'datasets\hazard5v3s\meta\hazard6_train.csv'),
        (Join-Path $workspaceRoot 'datasets\hazard5v3s\meta\hazard6_validation.csv'),
        (Join-Path $workspaceRoot 'datasets\hazard5v3s\meta\hazard6_development_test.csv'),
        (Join-Path $workspaceRoot 'datasets\hazard5v3s\meta\hazard6_quantization.csv')
    )) {
    ...
    $arguments = @(
        ...
        '--config-name', 'hazard5v3s_yamnet1024_tqe',
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "ST model-zoo YAMNet-1024 training failed with exit code $LASTEXITCODE"
        }
    }
    finally {
        Pop-Location
