...
try {
    ...
    $runRoot = Join-Path $workspaceRoot 'training-runs\hazard5r_yamnet256'
    $mlflowRoot = Join-Path $workspaceRoot 'training-runs\mlruns-hazard5r-short'

    foreach ($requiredPath in @(
        $python,
        (Join-Path $serviceRoot 'stm32ai_main.py'),
        (Join-Path $workspaceRoot 'datasets\hazard5r\audio'),
        (Join-Path $workspaceRoot 'datasets\hazard5r\meta\hazard5r_train.csv')
    )) {
    ...
    $arguments = @(
        ...
        '--config-name', 'hazard5r_bg2x_yamnet256_tqe',
