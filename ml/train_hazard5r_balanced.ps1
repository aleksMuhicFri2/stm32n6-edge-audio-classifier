...
try {
    ...
    $runRoot = Join-Path $workspaceRoot 'training-runs\hazard5r_balanced_yamnet256'
    $mlflowRoot = Join-Path $workspaceRoot 'training-runs\mlruns-hazard5r-balanced-short'

    foreach ($requiredPath in @(
        $python,
        (Join-Path $serviceRoot 'stm32ai_main.py'),
        (Join-Path $workspaceRoot 'datasets\hazard5r_balanced\audio'),
        (Join-Path $workspaceRoot 'datasets\hazard5r_balanced\meta\hazard5r_train.csv')
    )) {
    ...
    $arguments = @(
        ...
        '--config-name', 'hazard5r_balanced_yamnet256_tqe',
