param(
    [switch]$SmokeTest,
    [switch]$Balanced,
    [switch]$V3,
    [switch]$V3SplitSpeech
)

$ErrorActionPreference = 'Stop'
if (@($Balanced, $V3, $V3SplitSpeech).Where({ $_ }).Count -gt 1) {
    throw 'Training variants are separate controlled experiments.'
}
$longRepoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$projectRoot = (Resolve-Path (Join-Path $longRepoRoot '..')).Path
$shortDrive = 'T:'
if (Test-Path "$shortDrive\") {
    throw "Temporary training drive $shortDrive is already in use."
}

subst $shortDrive $projectRoot
try {
    $repoRoot = "$shortDrive\stm32n6-edge-audio-classifier"
    $workspaceRoot = "$shortDrive\ml-workspace"
    $serviceRoot = Join-Path $workspaceRoot 'stm32ai-modelzoo-services\audio_event_detection'
    $python = Join-Path $workspaceRoot '.venv\Scripts\python.exe'
    $configRoot = Join-Path $repoRoot 'ml\configs'
    $cacheRoot = Join-Path $workspaceRoot 'cache\matplotlib'
    $datasetName = if ($V3SplitSpeech) { 'hazard5v3r_split_speech' } elseif ($V3) { 'hazard5v3r' } elseif ($Balanced) { 'hazard5v2r_balanced' } else { 'hazard5v2r' }
    $configName = if ($V3SplitSpeech) { 'hazard5v3r_split_speech_yamnet256_tqe' } elseif ($V3) { 'hazard5v3r_bg2x_yamnet256_tqe' } else { 'hazard5v2r_bg2x_yamnet256_tqe' }
    $runName = if ($V3SplitSpeech) { 'hazard5v3r_split_speech_yamnet256' } elseif ($V3) { 'hazard5v3r_yamnet256' } elseif ($Balanced) { 'hazard5v2r_balanced_yamnet256' } else { 'hazard5v2r_yamnet256' }
    $mlflowName = if ($V3SplitSpeech) { 'mlruns-hazard5v3r-split-speech-short' } elseif ($V3) { 'mlruns-hazard5v3r-short' } elseif ($Balanced) { 'mlruns-hazard5v2r-balanced-short' } else { 'mlruns-hazard5v2r-short' }
    $runRoot = Join-Path $workspaceRoot "training-runs\$runName"
    $mlflowRoot = Join-Path $workspaceRoot "training-runs\$mlflowName"
    foreach ($requiredPath in @(
        $python,
        (Join-Path $serviceRoot 'stm32ai_main.py'),
        (Join-Path $workspaceRoot "datasets\$datasetName\audio"),
        (Join-Path $workspaceRoot "datasets\$datasetName\meta\hazard5r_train.csv")
    )) {
        if (-not (Test-Path $requiredPath)) {
            throw "Missing training prerequisite: $requiredPath"
        }
    }
    New-Item -ItemType Directory -Force $cacheRoot, $runRoot, $mlflowRoot | Out-Null
    $env:MPLCONFIGDIR = $cacheRoot
    $env:MPLBACKEND = 'Agg'
    $env:TF_CPP_MIN_LOG_LEVEL = '2'
    $env:TF_ENABLE_ONEDNN_OPTS = '0'
    $mlflowUri = [System.Uri]::new(
        $mlflowRoot + [System.IO.Path]::DirectorySeparatorChar
    ).AbsoluteUri
    $hydraRunDir = ($runRoot -replace '\\', '/') + '/${now:%Y_%m_%d_%H_%M_%S}'
    $arguments = @(
        'stm32ai_main.py',
        '--config-path', $configRoot,
        '--config-name', $configName,
        "mlflow.uri=$mlflowUri",
        "hydra.run.dir=$hydraRunDir"
    )
    if ($Balanced) {
        $datasetRel = "../../datasets/$datasetName"
        $arguments += "dataset.training_audio_path=$datasetRel/audio"
        $arguments += "dataset.training_csv_path=$datasetRel/meta/hazard5r_train.csv"
        $arguments += "dataset.validation_audio_path=$datasetRel/audio"
        $arguments += "dataset.validation_csv_path=$datasetRel/meta/hazard5r_validation.csv"
        $arguments += "dataset.test_audio_path=$datasetRel/audio"
        $arguments += "dataset.test_csv_path=$datasetRel/meta/hazard5r_development_test.csv"
        $arguments += "dataset.quantization_audio_path=$datasetRel/audio"
        $arguments += "dataset.quantization_csv_path=$datasetRel/meta/hazard5r_quantization.csv"
        $arguments += 'general.project_name=stm32n6_hazard5v2r_balanced_yamnet256_development'
    }
    if ($SmokeTest) {
        $arguments += 'training.epochs=1'
        $arguments += '+training.dryrun=1'
    }
    Push-Location $serviceRoot
    try {
        & $python @arguments
        if ($LASTEXITCODE -ne 0) {
            throw "ST model-zoo training failed with exit code $LASTEXITCODE"
        }
    }
    finally {
        Pop-Location
    }
}
finally {
    subst $shortDrive /D
}
