...
try {
    ...
    $runRoot = Join-Path $workspaceRoot "training-runs\useful10_yamnet256"
    $mlflowRoot = Join-Path $workspaceRoot "training-runs\mlruns-useful10-short"

    foreach ($requiredPath in @(
        $python,
        (Join-Path $serviceRoot "stm32ai_main.py"),
        (Join-Path $workspaceRoot "datasets\ESC-50\audio"),
        (Join-Path $workspaceRoot "datasets\ESC-50\meta\useful10_train.csv")
    )) {
    ...
    $arguments = @(
        ...
        "--config-name", "useful10_yamnet256_tqe",
