...
$manifestPath = Join-Path $reviewRoot "supplement_manifest.csv"
$responsePath = Join-Path $reviewRoot "supplement_responses.csv"

if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
    throw "Independent V4 supplement manifest is missing."
}
...
foreach ($trial in $trials) {
    ...
    if (-not (Test-Path -LiteralPath $audioPath -PathType Leaf)) {
        throw "Missing supplement audio: $audioPath"
    }
    ...
    if ($actualHash -ne $trial.stimulus_sha256.ToLowerInvariant()) {
        throw "Supplement hash mismatch for $($trial.candidate_id)."
    }
}
if ($ValidateOnly) {
    Write-Host "Validated all $($trials.Count) supplement files and hashes."
    exit 0
}

Write-Host "Blind semantic review of $($trials.Count) replacement candidates."
Write-Host "Only glass breaking, gunfire and sirens are included; previous accepted clips are preserved."
Write-Host "The board and model are not used, and responses are saved after every clip."
...
Write-Host "Supplement review complete. Tell Codex when it is done."
